...
+(instancetype)defaultConfiguration{
    ...
    if(config) {
        ...
        config.beforeSetCodeText=@"绘制解锁图案";//设置手势密码前的提示文字
        config.codeLengthErrorText=@"最少连接4个点，请重新输入";//密码长度低于最短长度的错误提示文字
        config.drawAgainText=@"再次绘制解锁图案";//确认手势手势密码，要求再次绘制的提示文字
